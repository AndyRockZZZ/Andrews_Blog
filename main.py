...
def login():
    ...
    if login_form.validate_on_submit():
        ...
        if get_user is None:
            ...
            return redirect(url_for("login"))
        elif not check_password_hash(pwhash=get_user.password, password=login_form.password.data):
            flash(message="Password incorrect, try again.")
            return redirect(url_for("login"))
        else:
            login_user(get_user)
            return redirect(url_for("get_all_posts"))
...
def show_post(post_id):
    ...
    list_comments = Comment.query.filter_by(post_id=post_id).all()
